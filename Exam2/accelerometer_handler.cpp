...
bool ReadAccelerometer(tflite::ErrorReporter* error_reporter, float* input, int *zaxis,
                       int length, bool reset_buffer) {
  // Clear the buffer if required, e.g. after a successful prediction
  if (reset_buffer) {
    memset(save_data, 0, 600 * sizeof(float));
    memset(zaxis_reg, 0, 600 * sizeof(int));
    begin_index = 0;
  ...
  for (int i = 0; i < length; ++i) {
    ...
    zaxis[i] = zaxis_reg[ring_array_index];
  }
